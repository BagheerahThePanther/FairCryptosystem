...
namespace DigitalSignatureGOST
{
    class GOSTSignature
    {
        ...
        public bool checkDigitalSignature(byte[] message, byte[] signature, byte[][] publicKey)
        {
           ...
           //e = BigInteger.Parse("20798893674476452017134061561508270130637142515379653289952617252661468872421");
            ...
            EllipticCurvePoint P = new EllipticCurvePoint(Px, Py);
            EllipticCurvePoint Q = new EllipticCurvePoint(new BigInteger(publicKey[0].Concat(new byte[] { 0 }).ToArray()), 
                                                          new BigInteger(publicKey[1].Concat(new byte[] { 0 }).ToArray()));
